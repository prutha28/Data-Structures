...
public class LinkedListUtils {
		...
		while( true )
		{
			if( current1 == null && current2 == null){
				if( carry == 0){
					...
					sumList.appendToTail( carry ) ; // When both lists have become empty, but have a carry over remaining.
				}
				return sumList.head ;
			}
	...
	/**
	 * Splits the source headed List into 2 sublists headed at ahead & bHead.
	 * @param source
	 * @param aHead
	 * @param bHead
	 */
	public static void alternatingSplit( Node sourceHead, Node aHead, Node bHead){
		...
		if( current == null )// empty list
			return ;
		else if( current.next == null ) // 1 node
			aCurrent = current ;
...
//		Node current1 = aCurrent ;
		Node current2 = bCurrent ;

		System.out.println("\nThe first sublist is :");

		while( aCurrent != null ){
			System.out.printf("%4d", aCurrent.data);
			aCurrent = aCurrent.next ;
		}
		System.out.println("\nThe second sublist is :");
		while( current2 != null ){
			System.out.printf("%4d", current2.data);
			current2 = current2.next ;
		}
	}
}
